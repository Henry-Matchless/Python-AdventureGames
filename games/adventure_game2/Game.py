...
class Game:
    ...
    def createGUI(self):
        ...
        self.south_button = tk.Button(self.root, bd=0, bg='black', activebackground='black',
                                     width=140, height=74, relief=tk.SOLID)       #创建按钮对象
        self.south_button.place(x=410, y=456, width=140, height=74)       #布置按钮位置
        try:
            img = ImageTk.PhotoImage(Image.open('res/south.png').resize((140, 74), Image.ANTIALIAS))   #缩放图片
            self.imglist['south'] = img    #添加图片对象到列表
            self.south_button.config(image=img)       #关联按钮图案
        except:
            pass
        self.south_button.place(x=410, y=456, width=0, height=0)       #隐藏按钮位置
        #north
        self.north_button = tk.Button(self.root, bd=0, bg='black', activebackground='black',
                                     width=140, height=74, relief=tk.SOLID)       #创建按钮对象
        self.north_button.place(x=410, y=10, width=140, height=74)       #布置按钮位置
        try:
            img = ImageTk.PhotoImage(Image.open('res/north.png').resize((140, 74), Image.ANTIALIAS))   #缩放图片
            self.imglist['north'] = img    #添加图片对象到列表
            self.north_button.config(image=img)       #关联按钮图案
        except:
            pass
        self.north_button.place(x=410, y=10, width=0, height=0)       #隐藏按钮位置
        #east
        self.east_button = tk.Button(self.root, bd=0, bg='black', activebackground='black',
                                     width=72, height=140, relief=tk.SOLID)       #创建按钮对象
        self.east_button.place(x=876, y=210, width=72, height=140)       #布置按钮位置
        try:
            img = ImageTk.PhotoImage(Image.open('res/east.png').resize((72, 140), Image.ANTIALIAS))   #缩放图片
            self.imglist['east'] = img    #添加图片对象到列表
            self.east_button.config(image=img)       #关联按钮图案
        except:
            pass
        self.east_button.place(x=876, y=210, width=0, height=0)       #隐藏按钮位置
        #west
        self.west_button = tk.Button(self.root, bd=0, bg='black', activebackground='black',
                                     width=72, height=140, relief=tk.SOLID)       #创建按钮对象
        self.west_button.place(x=10, y=210, width=72, height=140)       #布置按钮位置
        try:
            img = ImageTk.PhotoImage(Image.open('res/west.png').resize((72, 140), Image.ANTIALIAS))   #缩放图片
            self.imglist['west'] = img    #添加图片对象到列表
            self.west_button.config(image=img)       #关联按钮图案
        except:
            pass
        self.west_button.place(x=10, y=210, width=0, height=0)       #隐藏按钮位置        
        #物品对象
        self.thing = tk.Button(self.root, bd=0, width=100, height=100, relief=tk.SOLID)  #物品对象
        self.thing.place(x=720, y=150, width=100, height=100)      #布置到界面上(右上方)
        #怪物对象
        ...
        try:
            ...
            pass
        #人物对象
        self.player = tk.Label(self.root, bd=0, width=60, height=80)  #人物对象
        self.player.place(x=450, y=210, width=60, height=80)      #布置到界面上(正中)
        try:
            img = ImageTk.PhotoImage(Image.open('res/player.png').resize((60, 80), Image.ANTIALIAS))   #缩放图片
...
##                print(str(e))
            ...
            if "upstairs" in exits or "north" in exits:
                ...
                if "upstairs" in exits:
                    command = ('go', 'upstairs')
                    self.north_button.config(command=partial(self.move, [450, 50], command))
                else:
                    command = ('go', 'north')
                    self.north_button.config(command=partial(self.move, [450, 50], command))
            else:
            ...
            if "downstairs" in exits or "south" in exits:
                ...
                if "downstairs" in exits:
                    ...
                    self.south_button.config(command=partial(self.move, [450, 410], command))
                else:
                    command = ('go', 'south')
                    self.south_button.config(command=partial(self.move, [450, 410], command))
            else:
            ...
            if "east" in exits:
                self.east_button.place(x=876, y=210, width=72, height=140)       #布置按钮位置
                command = ('go', 'east')
                self.east_button.config(command=partial(self.move, [830, 240], command))
            else:
                self.east_button.place(x=876, y=210, width=0, height=0)       #隐藏按钮位置
            if "west" in exits:
                self.west_button.place(x=10, y=210, width=72, height=140)       #布置按钮位置
                command = ('go', 'west')
                self.west_button.config(command=partial(self.move, [60, 240], command))
            else:
                self.west_button.place(x=10, y=210, width=0, height=0)       #隐藏按钮位置
            #修改房间标签            
            ...
            if self.currentRoom.things:
                ...
                try:
                    ...
                    if os.path.exists(imgpath):
                        img = ImageTk.PhotoImage(Image.open(imgpath).resize((100, 100), Image.ANTIALIAS))   #缩放图片
                    else:
                        img = ImageTk.PhotoImage(Image.open('res/bthing.png').resize((100, 100), Image.ANTIALIAS))   #缩放图片
...
##                    print(str(e))
                ...
                self.thing.place(x=temp.location[0], y=temp.location[1], width=100, height=100)      #布置到界面上
            else:
        ...
        if self.move_flag:
            ...
            if self.player_location[0]<=self.location[0]+abs(self.mx)+0.1 and self.player_location[1]<=self.location[1]+abs(self.my)+0.1 \
               and self.player_location[0]>=self.location[0]-abs(self.mx)-0.1 and self.player_location[1]>=self.location[1]-abs(self.my)-0.1:   #到达目标位置
                ...
                return
            #更新人物位置
            self.player.place(x=self.player_location[0], y=self.player_location[1], width=60, height=80)
            #执行动画效果
...
##                messagebox.showinfo(title='warn', message=self.currentRoom.getLongDescription()+'\n')
            ...
            if secondWord == 'upstairs' or secondWord == 'north':   #从上方进门,出来在最下
                self.player.place(x=450, y=410, width=60, height=80)      #布置到界面上(最下)
                self.player_location = [450, 410]
            elif secondWord == 'downstairs' or secondWord == 'south':   #从下方进门,出来在最上
                self.player.place(x=450, y=50, width=60, height=80)      #布置到界面上(最上)
                self.player_location = [450, 50]
            elif secondWord == 'west':          #从左方进门,出来在最右
                self.player.place(x=830, y=240, width=60, height=80)      #布置到界面上(最右)
                self.player_location = [830, 240]
            elif secondWord == 'east':          #从右方进门,出来在最左
                self.player.place(x=60, y=240, width=60, height=80)      #布置到界面上(最左)
                self.player_location = [60, 240]
    ...
    def doPickUpCommand(self, secondWord): # If the place has supplies then you can pick it up and store it in your package.
        ...
        if self.currentRoom.isEmpty():
            ...
            #移动人物到宝箱位置
            if secondWord:
                self.player.place(x=secondWord[0], y=secondWord[1], width=60, height=80)      #布置到物品所在位置
